...
class Order_pembelian_barang_m extends CI_Model
{
    ...
    function get_pembelian($keyword, $id_klien){
        ...
        $sql = "
        SELECT * FROM ak_pembelian WHERE $where_unit AND TIPE = 'OPB'
        ORDER BY ID DESC
        ";
    ...
    function get_pembelian_filter($keyword, $id_klien, $tgl_awal, $tgl_akhir){
        ...
        $sql = "
        SELECT * FROM ak_pembelian WHERE $where AND $where_unit AND TIPE = 'OPB'
        ORDER BY ID DESC
        ";
    ...
    function simpan_detail_pembelian($id_penjualan, $id_klien, $nama_produk, $qty, $satuan, $harga_satuan, $jumlah, $kode_akun, $keterangan, $id_produk, $no_spb){
        ...
        (ID_KLIEN, ID_PENJUALAN, NAMA_PRODUK, QTY, SATUAN, HARGA_SATUAN, TOTAL, KODE_AKUN, KETERANGAN, ID_PRODUK, NO_SPB)
        VALUES 
    ...
    function simpan_order_pembelian_barang($unit, $no_opb, $tgl_trx, $uraian, $unit){
        $total_all       = str_replace(',', '', $total_all);
        $sql = "
        INSERT INTO ak_pembelian 
        (NO_BUKTI, TGL_TRX, TIPE, MEMO, UNIT)
        VALUES 
        ('$no_opb', '$tgl_trx', 'OPB', '$uraian', '$unit')
        ";
