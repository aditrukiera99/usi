...
class Lap_penerimaan_barang_m extends CI_Model
{
    ...
    function get_pembelian($keyword, $id_klien){
        ...
        $sql = "
        SELECT * FROM ak_pembelian WHERE $where_unit AND TIPE = 'LPB'
        ORDER BY ID DESC
        ";
    ...
    function get_pembelian_filter($keyword, $id_klien, $tgl_awal, $tgl_akhir){
        ...
        $sql = "
        SELECT * FROM ak_pembelian WHERE $where AND $where_unit AND TIPE = 'LPB'
        ORDER BY ID DESC
        ";
    ...
    function simpan_order_pembelian_barang($unit, $no_opb, $tgl_trx, $uraian, $unit, $id_pelanggan, $pelanggan, $no_po){
        $total_all       = str_replace(',', '', $total_all);
        $sql = "
        INSERT INTO ak_pembelian 
        (NO_BUKTI, TGL_TRX, TIPE, MEMO, UNIT, ID_PELANGGAN, PELANGGAN, NO_PO)
        VALUES 
        ('$no_opb', '$tgl_trx', 'LPB', '$uraian', '$unit', '$id_pelanggan', '$pelanggan', '$no_po')
        ";
