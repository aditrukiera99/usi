...
class Lap_mutasi_hutang_m extends CI_Model
{
    ...
    function get_hutang_bulanan($bulan, $tahun, $unit){
        ...
        LEFT JOIN (
            SELECT a.KONTAK, IFNULL(IFNULL(a.HUTANG_LALU, 0) - IFNULL(a.MUTASI_LALU,0), 0) AS SALDO_AWAL FROM (
                SELECT a.KONTAK, SUM(a.HUTANG_LALU) AS HUTANG_LALU, SUM(a.MUTASI_LALU) AS MUTASI_LALU FROM (
                    ...
                    WHERE a.UNIT = '$unit' AND STR_TO_DATE(a.TGL, '%d-%c-%Y') < STR_TO_DATE('01-$bulan-$tahun' , '%d-%c-%Y') AND (b.KODE_AKUN LIKE '%200%' OR b.KODE_AKUN LIKE '%260%')
                    GROUP BY a.KONTAK
                    ...
                    AND (KODE_AKUN LIKE '%200%' OR KODE_AKUN LIKE '%260%')
                    AND TIPE  = 'HUTANG'
                    GROUP BY KONTAK
                ) a
                GROUP BY a.KONTAK
        ...
        LEFT JOIN (
            ...
            AND (b.KODE_AKUN LIKE '%200%' OR b.KODE_AKUN LIKE '%260%')
            GROUP BY a.KONTAK
        ) d ON a.NAMA_SUPPLIER = d.KONTAK
        ...
        LEFT JOIN (
            SELECT a.KONTAK, IFNULL(IFNULL(a.HUTANG_LALU, 0) - IFNULL(a.MUTASI_LALU,0), 0) AS SALDO_AWAL FROM (
                SELECT a.KONTAK, SUM(a.HUTANG_LALU) AS HUTANG_LALU, SUM(a.MUTASI_LALU) AS MUTASI_LALU FROM (
                    ...
                    FROM ak_input_voucher a
                    JOIN ak_input_voucher_detail b ON a.NO_VOUCHER = b.NO_VOUCHER_DETAIL
                    WHERE a.UNIT = '$unit' AND STR_TO_DATE(a.TGL, '%d-%c-%Y') < STR_TO_DATE('01-01-$tahun' , '%d-%c-%Y') AND (b.KODE_AKUN LIKE '%200%' OR b.KODE_AKUN LIKE '%260%')
                    GROUP BY a.KONTAK
