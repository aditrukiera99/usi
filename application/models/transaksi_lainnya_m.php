...
class Transaksi_lainnya_m extends CI_Model
{
    ...
    function get_transaksi_lainnya($keyword, $id_klien){
        ...
        $sql = "
        SELECT * FROM ak_transaksi_lainnya WHERE ID_KLIEN = $id_klien AND $where_unit
        ORDER BY ID DESC
        ";
    ...
    function get_data_trx($id){
        $sql = "
        SELECT a.*, b.NAMA_UNIT FROM ak_transaksi_lainnya a
    ...
    function get_data_trx_detail($id){
        $sql = "
        SELECT * FROM ak_transaksi_lainnya_detail WHERE ID_PENJUALAN = $id
        ";
    ...
    function hapus_trx_penjualan($id_hapus){
        $sql_1 = "
        DELETE FROM ak_transaksi_lainnya WHERE ID = $id_hapus
        ";
    ...
    function get_penjualan_filter($keyword, $id_klien, $tgl_awal, $tgl_akhir){
        ...
        $sql = "
        SELECT * FROM ak_transaksi_lainnya WHERE ID_KLIEN = $id_klien AND $where AND $where_unit
        ORDER BY ID DESC
        ";
    ...
    function get_no_trx_penjualan($id_klien){
        $sql = "
        SELECT * FROM ak_nomor WHERE ID_KLIEN = $id_klien AND TIPE = 'Trf Lainnya'
        ";
    ...
    function simpan_penjualan($id_klien, $no_trx, $id_pelanggan, $pelanggan, $alamat_tagih, $tgl_trx, $tgl_jatuh_tempo, $id_pajak, $sub_total, $pajak_total, 
                              $total_all, $sts_lunas, $memo_lunas, $akun_piutang, $kode_akun_pajak, $unit)
    {
        ...
        $sql = "
        INSERT INTO ak_transaksi_lainnya 
        (ID_KLIEN, NO_BUKTI, ID_PELANGGAN, PELANGGAN, ALAMAT, TGL_TRX, TGL_JATUH_TEMPO, ID_PAJAK, SUB_TOTAL, NILAI_PAJAK, TOTAL, LUNAS, MEMO, KODE_AKUN_PIUTANG, KODE_AKUN_PAJAK, UNIT)
        VALUES 
        ('$id_klien', '$no_trx', '$id_pelanggan', '$pelanggan', '$alamat_tagih', '$tgl_trx', '$tgl_jatuh_tempo', '$id_pajak', '$sub_total', '$pajak_total', '$total_all', '$sts_lunas', '$memo_lunas', '$akun_piutang', '$kode_akun_pajak', '$unit')
        ";
    ...
    function ubah_penjualan($id, $no_trx, $id_pelanggan, $pelanggan, $alamat_tagih, $tgl_trx, $tgl_jatuh_tempo, $id_pajak, $sub_total, $pajak_total, $total_all, $sts_lunas, $memo_lunas, $akun_piutang, $kode_akun_pajak){
         $sql = "
            UPDATE ak_transaksi_lainnya SET 
                ...
                ID_PAJAK = '$id_pajak', 
                SUB_TOTAL = '$sub_total', 
                NILAI_PAJAK = '$pajak_total',
                TOTAL = '$total_all', 
                LUNAS = '$sts_lunas', 
                MEMO = '$memo_lunas', 
                KODE_AKUN_PIUTANG = '$akun_piutang', 
                KODE_AKUN_PAJAK = '$kode_akun_pajak'    
            WHERE ID = '$id'
    ...
    function simpan_detail_trx($id_penjualan, $id_klien, $nama_produk, $qty, $satuan, $harga_satuan, $jumlah, $kode_akun){
        ...
        $sql = "
        INSERT INTO ak_transaksi_lainnya_detail 
    ...
    function hapus_detail_trx($id){
        $sql = "
        DELETE FROM ak_transaksi_lainnya_detail WHERE ID_PENJUALAN = $id
        ";
