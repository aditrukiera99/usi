...
class Surat_penawaran_harga_m extends CI_Model
{
	...
	{
		$sql = "
			INSERT INTO tb_surat_penawaran_harga (
				no_spb,
				tanggal,
				uraian,
				divisi,
	...
	function simpan_data_barang_detail($id_permintaan_baru,$id_produk, $nama_produk,$keterangan,$kuantitas,$satuan)
	{
		...
		$sql = "
			INSERT INTO tb_surat_penawaran_harga_detail (
				id_induk,
				id_produk,
				nama_produk,
				keterangan,
	...
	{
		$sql = "
			SELECT * FROM tb_surat_penawaran_harga ";
	...
	{
		$sql = "DELETE FROM  tb_surat_penawaran_harga WHERE id_permintaan = '$id' " ;
		$this->db->query($sql);

		$sql2 = "DELETE FROM  tb_surat_penawaran_harga_detail WHERE id_induk = '$id' " ;
		$this->db->query($sql2);
	}

	function data_permintaan_id($id)
	{
		$sql = "SELECT * FROM tb_surat_penawaran_harga WHERE id_permintaan = '$id' ";
	...
	{
		$sql = "SELECT * FROM tb_surat_penawaran_harga_detail WHERE id_induk = '$id' ";
		$query = $this->db->query($sql);
		return $query->result();
	}
	...
	{
		$sql = "
			UPDATE tb_surat_penawaran_harga SET 
				no_spb  	= '$no_spb',
				tanggal 	= '$tanggal',
				uraian  	= '$uraian'
	...
	function ubah_data_permintaan_detail($id,$nama_produk,$keterangan,$kuantitas,$satuan,$harga,$jumlah)
	{
		...
		$sql = "
			UPDATE tb_surat_penawaran_harga_detail SET 
    ...
    function get_data_trx($id){
    	$sql = "
        SELECT pb.* , md.nama_divisi FROM tb_surat_penawaran_harga pb , master_divisi md WHERE pb.divisi = md.id_divisi AND pb.id_permintaan = '$id'
        ";
    ...
    function get_data_trx_detail($id){
    	$sql = "
        SELECT * FROM tb_surat_penawaran_harga_detail WHERE id_induk = '$id'
        ";
