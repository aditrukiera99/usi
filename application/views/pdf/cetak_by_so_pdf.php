...
<?php 
  $nomer_so = $dt->NOMER_SO;
  $qty_do = $this->db->query("SELECT * FROM ak_delivery_order WHERE NO_SO = '$nomer_so' ")->row();
...
<div style="width: 100%;padding-top: 10px;padding-bottom: 10px;padding-left:10px; ">
  <table style="width: 100%;">
    ...
    <tr>
      ...
      <?php 
        $id_pelang = $dt_deti->ID_PELANGGAN;
        $sql_holding = $this->db->query("SELECT * FROM ak_pelanggan WHERE ID = '$id_pelang'")->row();

      ?>
      <td  style="width:40%;text-align:left;font-size: 15px;"><?=$sql_holding->NAMA_USAHA;?><br><?=$dt_deti->ALAMAT;?></td>
    </tr>
    <tr>
