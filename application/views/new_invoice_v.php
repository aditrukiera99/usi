...
<div class="row-fluid ">
	<div class="span12">
		<div class="primary-head">
			<h3 class="page-header"> <i class="icon-angle-right"></i> Buat Invoice </h3>
		</div>
		<ul class="breadcrumb">
		...
		<div class="control-group">
			...
			<div class="controls">
				<div class="input-append">
					<input type="text" id="no_sales_order" name="no_sales_order" readonly style="background:#FFF; width: 70%;">
		...
		<div class="content-widgets light-gray">
			...
			<div class="widget-container">
				...
				<table class="stat-table table table-hover">
					...
					<tbody id="tes">
						<tr id="info_none">
							<td colspan="8" style="text-align: center; font-size: 14px;"> Silahkan pilih Sales order </td>
						</tr>
					</tbody>
				</table>
...
function ajax_spb(){
    ...
    $.ajax({
        url : '<?php echo base_url(); ?>invoice_c/get_all_spb',
        type : "POST",
        dataType : "json",
        data : {
            keyword : keyword,
        ...
        success : function(result){
            ...
            $.each(result,function(i,res){
                no++;
                isine += '<tr onclick="get_more('+res.ID+', &quot;'+res.NO_BUKTI+'&quot;);" style="cursor:pointer;">'+
...
function get_more(id_spb, no_bukti){
	$('#popup_load').show();
	$('#no_sales_order').val(no_bukti);
	$.ajax({
		url : '<?php echo base_url(); ?>invoice_c/get_more',
		data : {id_spb:id_spb},
		type : "POST",
		dataType : "json",
		success : function(result){
			...
			$.each(result,function(i,res){
				...
				'<tr id="tr_'+i+'" class="tr_utama">'+
					...
					'<td class="center" align="center" style="vertical-align:middle;"> '+
						'<div class="controls">'+
							'<input style="font-size: 14px; text-align:left;" type="text"   name="keterangan[]" id="keterangan_'+i+'" value="">'+
						'</div>'+
					'</td>'+
