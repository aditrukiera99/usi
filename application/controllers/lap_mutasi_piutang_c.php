...
class Lap_mutasi_piutang_c extends CI_Controller {
	...
	function __construct()
	{
	    ...
	    $this->load->model('lap_mutasi_piutang_m','model');
	...
	function index()
	{
		...
		$data =  array(
			'page' => "lap_mutasi_piutang_v", 
			'title' => "Laporan Mutasi Piutang", 
			'msg' => "", 
			'master' => "laporan", 
			'view' => "mutasi_piutang", 
			//'dt' => $dt, 
	...
	function cetak_penye_xls(){
		...
		$view = "xls/report_mutasi_piutang_xls";
		$dt = "";
		$judul = "";
		$dt_aktiva = "";
		$dt_wajib = "";
		...
		if($filter == "Bulanan"){
			...
			$dt    = $this->model->get_hutang_bulanan($bulan, $tahun, $unit);			
		} else if($filter == "Tahunan"){
			$tahun = $this->input->post('tahun2');
			$bulan = 0;
			$bln_txt = $this->datetostr($bulan);
			$judul = "Tahun $tahun";
			$dt    = $this->model->get_hutang_tahunan($tahun, $unit);
		} 
	...
	function cetak_penye_pdf(){
		...
		$filter = $this->input->post('filter');
		$unit = $this->input->post('unit');
		$view = "pdf/report_mutasi_piutang_pdf";
		$dt = "";
		$judul = "";
		$dt_aktiva = "";
		$dt_wajib = "";
