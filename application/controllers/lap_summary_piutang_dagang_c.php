...
class Lap_summary_piutang_dagang_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			...
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_summary_piutang_dagang_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/lap_summary_piutang_dagang_pdf";
			$dt = "";
			...
			$sql = "
				SELECT
					...
					a.TOTAL_DO,
					a.SUB_TOTAL,
					IFNULL((a.TOTAL_DO - a.SUB_TOTAL),0) AS SALDO_BLN_LALU
				FROM(
					SELECT
						a.ID,
						a.ID_PELANGGAN,
