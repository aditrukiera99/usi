...
class Lap_hutang_jatuh_tempo_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			...
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_hutang_jatuh_tempo_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/lap_hutang_jatuh_tempo_pdf";
			$dt = "";
			...
			$sql = "
				...
				FROM ak_pembelian a
				LEFT JOIN ak_supplier b ON b.ID = a.ID_PELANGGAN
