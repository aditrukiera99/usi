...
class Lap_daftar_customer_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			...
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_daftar_customer_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/lap_daftar_customer_pdf";
			$dt = "";
			...
			$dt = $this->db->query("SELECT * FROM ak_pelanggan ORDER BY ID")->result();
		} else {
			$view = "pdf/lap_daftar_customer_pdf";
			$dt = "";
			$dt_unit = $this->master_model_m->get_unit_by_id($unit);
