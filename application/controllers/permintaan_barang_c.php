...
class Permintaan_barang_c extends CI_Controller {
	...
	function __construct()
	{
	    ...
	    $this->load->model('permintaan_barang_m','model');
	...
	function index()
	{
		...
		if($this->input->post('simpan')){
			...
			$this->model->simpan_permintaan_barang($unit, $no_spb, $tgl_trx, $uraian, $user->UNIT);
			...
			foreach ($nama_produk as $key => $val) {
				$this->model->simpan_detail_pembelian($last_id, $id_klien, $val, $qty[$key], $satuan[$key], $harga_satuan[$key], $jumlah[$key], $kode_akun[$key], $keterangan[$key], $id_produk[$key]);
