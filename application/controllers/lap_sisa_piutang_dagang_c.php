...
class Lap_sisa_piutang_dagang_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			'page' => "lap_sisa_piutang_dagang_v", 
			'title' => "Laporan Sisa Piutang Dagang", 
			'msg' => "", 
			'master' => "laporan_penjualan", 
			'view' => "lap_sisa_piutang_dagang_v", 
			//'dt' => $dt, 
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_sisa_piutang_dagang_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/lap_sisa_piutang_dagang_pdf";
			$dt = "";
