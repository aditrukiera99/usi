...
class Transaksi_lainnya_c extends CI_Controller {
	...
	function __construct()
	{
	    ...
	    $this->load->model('transaksi_lainnya_m','model');
	...
	function index()
	{
		...
		$dt = $this->model->get_transaksi_lainnya($keyword, $id_klien);
		...
		if($this->input->post('cari')){
			...
			$this->master_model_m->simpan_log($id_user, "Menghapus transaksi lainnya dengan nomor transaksi : <b>".$get_data_trx->NO_BUKTI."</b>");
		}


		$data =  array(
			'page' => "transaksi_lainnya_v", 
			'title' => "Transaksi Lainnya",  
			'master' => "input_data", 
			'view' => "transaksi_lainnya", 
			'dt' => $dt, 
			'msg' => $msg, 
			'tgl_full' => $tgl_full, 
			'kode_produk' => $kode_produk, 
			'post_url' => 'transaksi_lainnya_c', 
	...
	function new_invoice(){
		...
		if($this->input->post('simpan')){
			...
			foreach ($nama_produk as $key => $val) {
				$this->model->simpan_detail_trx($id_penjualan, $id_klien, $val, $qty[$key], $satuan[$key], $harga_satuan[$key], $jumlah[$key], $kode_akun[$key]);
			}


			$this->model->save_next_nomor($id_klien, 'Trf Lainnya', $no_trx2);
			$this->master_model_m->simpan_log($id_user, "Melakukan transaksi lainnya dengan nomor transaksi : <b>".$no_trx."</b>");
		...
		$data =  array(
			'page' => "buat_transaksi_lainnya_new_v", 
			...
			'get_pajak' => $get_pajak, 
			'no_trx' => $no_trx, 
			'post_url' => 'transaksi_lainnya_c/new_invoice', 
		);
	...
	function detail($id=""){
		...
		if($this->input->post('ubah')){
			...
			$no_trx2          =   $this->input->post('no_trx2');
			$id_pajak         =   $this->input->post('id_pajak');
			$sub_total        =   str_replace(',', '', $this->input->post('nilai_transaksi'));
			$pajak_total      =   str_replace(',', '', $this->input->post('pajak_all'));
			$total_all        =   str_replace(',', '', $this->input->post('nilai_transaksi'));
			$sts_lunas        =   $this->input->post('sts_lunas');
			$akun_piutang     =   $this->input->post('akun_piutang');
			$kode_akun_pajak  =   $this->input->post('kode_akun_pajak');
			$memo_lunas       =   addslashes($this->input->post('memo_lunas'));
			...
			$this->model->hapus_detail_trx($id);
			foreach ($nama_produk as $key => $val) {
				$this->model->simpan_detail_trx($id, $id_klien, $val, $qty[$key], $satuan[$key], $harga_satuan[$key], $jumlah[$key], $kode_akun[$key]);
			}

			$this->master_model_m->simpan_log($id_user, "Mengubah transaksi lainnya dengan nomor transaksi : <b>".$no_trx."</b>");
		}
		...
		$data =  array(
			'page' => "detail_transaksi_lainnya_v", 
			...
			'get_pel_sup' => $get_pel_sup, 
			'get_pajak' => $get_pajak, 
			'post_url' => 'transaksi_lainnya_c/detail/'.$id, 
		);
	...
	function get_pelanggan_popup(){
		...
		$sql = "
		SELECT * FROM ak_pelanggan WHERE ID_KLIEN = $id_klien AND $where AND APPROVE = 3
		";
