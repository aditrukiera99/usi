...
class Lap_history_harga_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			...
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_history_harga_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/report_history_harga_pdf";
			$dt = "";
			...
			$sql = "
				SELECT
					...
					b.SUPPLIER,
					a.HARGA_SATUAN,
					(a.HARGA_SATUAN - ((a.HARGA_SATUAN * 10) / 100)) AS HRG_TNP_PPN,
					b.NOMER_LPB,
					b.TGL_TRX
				FROM ak_penerimaan_detail a
				LEFT JOIN ak_penerimaan_barang b ON b.ID = a.ID_PENJUALAN
				WHERE STR_TO_DATE(b.TGL_TRX, '%d-%c-%Y') <= STR_TO_DATE('$tgl_akhir' , '%d-%c-%Y') 
