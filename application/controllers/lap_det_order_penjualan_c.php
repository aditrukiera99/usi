...
class Lap_det_order_penjualan_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			'page' => "lap_det_order_penjualan_v", 
			'title' => "Laporan Detail Order Penjualan", 
			'msg' => "", 
			'master' => "laporan", 
			'view' => "lap_det_order_penjualan_v", 
			//'dt' => $dt, 
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_det_order_penjualan_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/lap_det_order_penjualan_pdf";
			$dt = "";
