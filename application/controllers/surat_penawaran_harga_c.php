...
class Surat_penawaran_harga_c extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('surat_penawaran_harga_m','permintaan');
	...
	public function index()
	{
		$data = array(
				...
				'url_simpan' 	  => base_url().'surat_penawaran_harga_c/simpan',
				'url_hapus'  	  => base_url().'surat_penawaran_harga_c/hapus',
			);
	...
	function simpan()
	{
		...
		if ($id_permintaan == '') {
			...
			$get_nomor	   = $this->master_model_m->get_nomor_dokumen('SURAT_PENAWARAN_HARGA');

			$no_bukti_real 		= $get_nomor."/SPH/".$dept_row->nama_divisi."/".$var."/".$tahun_kas;

			$no_spb 	  = $this->input->post('no_spb');
			$tanggal 	  = $this->input->post('tanggal');
			$uraian 	  = $this->input->post('uraian');

			$this->master_model_m->update_nomor('SURAT_PENAWARAN_HARGA');
			...
			redirect('surat_penawaran_harga_c');
	...
	function cetak($id=""){
		...
		$data =  array(
			'page' => "surat_penawaran_harga_c", 
			'dt' => $dt,
			'dt_det' => $dt_det,
		);
		
		$this->load->view('pdf/report_surat_surat_penawaran_harga_pdf', $data);
	}
}
