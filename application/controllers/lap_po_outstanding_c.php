...
class Lap_po_outstanding_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			...
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_po_outstanding_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/report_po_outstanding_pdf";
			$dt = "";
			...
			$sql = "
				SELECT
					...
					a.NO_PO,
					a.SUPPLIER,
					a.TGL_TRX AS TANGGAL,
					b.TGL_TRX AS TGL_BTH,
					a.SUPPLIER
				FROM ak_penerimaan_barang a
				LEFT JOIN ak_pembelian b ON b.NO_PO = a.NO_PO
