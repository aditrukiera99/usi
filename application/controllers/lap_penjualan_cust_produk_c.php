...
class Lap_penjualan_cust_produk_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			'page' => "lap_penjualan_cust_produk_v", 
			'title' => "Laporan Penjualan Per Produk", 
			'msg' => "", 
			'master' => "laporan_penjualan", 
			'view' => "lap_penjualan_cust_produk_v", 
			//'dt' => $dt, 
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_penjualan_cust_produk_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/lap_penjualan_cust_produk_pdf";
			$dt = "";
