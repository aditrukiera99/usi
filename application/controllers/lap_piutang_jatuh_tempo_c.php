...
class Lap_piutang_jatuh_tempo_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			...
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_piutang_jatuh_tempo_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			$view = "pdf/lap_piutang_jatuh_tempo_pdf";
			$dt = "";
			...
			$sql = "
				...
				FROM(
					SELECT
						...
						a.NOMER_INV,
						a.KUANTITAS,
						b.TGL_TRX AS TGL_TRX_DO,
						b.PRODUK,
						b.QTY,
						b.HARGA_SATUAN,
						(b.QTY * b.HARGA_SATUAN) AS TOTAL_DO
					FROM ak_penjualan a
					LEFT JOIN ak_delivery_order b ON b.NO_SO = a.NO_BUKTI
