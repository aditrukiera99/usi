...
class Lap_penjualan_produk_detail_cust_c extends CI_Controller {
	...
	function index()
	{
		...
		$data =  array(
			...
			'msg' => $msg, 
			'user' => $user, 
			'post_url' => 'lap_penjualan_produk_detail_cust_c/cetak_laporan', 
		);
	...
	function cetak_laporan_pdf(){
		...
		if($filter == "Harian"){
			...
			$sql = "
				...
				FROM(
					SELECT
						...
						a.HARGA_SATUAN,
						a.HARGA,
						SUM(a.JML) AS JML_BLN_LALU,
						SUM(a.NILAI) AS NILAI_BLN_LALU,
						'0' AS JML_BLN_SKG,
						'0' AS NILAI_BLN_SKG
					FROM(
						SELECT
					...
					SELECT
						...
						'0' AS JML_BLN_LALU,
						'0' AS NILAI_BLN_LALU,
						SUM(a.JML) AS JML_BLN_SKG,
						SUM(a.NILAI) AS NILAI_BLN_SKG
					FROM(
						SELECT
							a.ID,
							a.TGL_TRX,
			...
			$view = "pdf/lap_penjualan_produk_detail_cust_bulanan_pdf";
			$dt = "";
			$dt_unit = $this->master_model_m->get_unit_by_id($unit);
			...
			$sql = "
				...
				FROM(
					...
					FROM(
						...
						FROM ak_delivery_order a
						LEFT JOIN ak_produk b ON b.ID = a.ID_PRODUK
						WHERE a.TGL_TRX LIKE '%-$bulan_txt2-$tahun%'
					) a
					GROUP BY a.PRODUK, a.KODE_PRODUK
